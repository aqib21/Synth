    ...
    public partial class Form_AgentAddEdit : Form
    {
        ...
        private void Button_Save_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (agent_id_edit == 0)
                {
                    using var cmd = new NpgsqlCommand($"SELECT * FROM FN_InsertAgent('{name}')", conn);
                    cmd.ExecuteNonQuery();
                }
                else
                {
                    using var cmd = new NpgsqlCommand($"SELECT * FROM FN_UpdateAgent('{name}', {agent_id_edit})", conn);
                    cmd.ExecuteNonQuery();
                }
            }
            catch (Exception ex)
        ...
        private void Form_AgentAddEdit_Load(object sender, EventArgs e)
        {
            if (agent_id_edit != 0)
            {
                ...
                {
                    using var conn = DB.GetConnection();
                    using var cmd = new NpgsqlCommand($"SELECT * FROM FN_GetAgent({agent_id_edit})", conn);
